...
public class InvoiceController {
    ...
    public InvoiceController(InvoiceService invoiceService, JwtUtils jwtUtils,
            ReturnInvoviceService returnInvoviceService, VNPAYService vnPayService) {
    ...
    public ResponseEntity<MessageResponse> createInvoice(@RequestParam("showTimeId") Long showTimeId,
        ...
        try {
            ...
            return ResponseEntity
                    .ok(new MessageResponse("Tạo hóa đơn thành công", HttpStatus.OK.value(), Instant.now().toString()));
    ...
    public ResponseEntity<PageResponse<InvoiceDto>> getAllInvoices(
        ...
        invoices.setContent(invoiceService.getAllInvoices(page, size, invoiceCode, cinemaId, roomId, movieId,
                showTimeCode, staffId, userId, startDate, endDate));
        invoices.setTotalElements(invoiceService.countAllInvoices(invoiceCode, cinemaId, roomId, movieId, showTimeCode,
                staffId, userId, startDate, endDate));
    ...
    public ResponseEntity<MessageResponse> cancelInvoice(@RequestParam("invoiceId") Long invoiceId,
        ...
        try {
            ...
            return ResponseEntity
                    .ok(new MessageResponse("Hủy hóa đơn thành công", HttpStatus.OK.value(), Instant.now().toString()));
        } catch (AppException e) {
            return ResponseEntity.status(e.getStatus())
                    .body(new MessageResponse(e.getMessage(), e.getStatus(), e.getTimestamp()));
        }
    ...
    public ResponseEntity<MessageResponse> submitOrder(@RequestParam("amount") int orderTotal,
        ...
        String vnpayUrl = vnPayService.createOrder(request, orderTotal, showTimeId, seatIds, foodIds, emailUser,
                staffId);
    ...
    public ResponseEntity<MessageResponse> paymentCompleted(HttpServletRequest request) {
        ...
        if (paymentStatus == 1) {
            return ResponseEntity.ok().body(
                    new MessageResponse("Thanh toán thành công", HttpStatus.OK.value(), Instant.now().toString()));
        } else {
            return ResponseEntity.badRequest().body(new MessageResponse("Thanh toán thất bại",
